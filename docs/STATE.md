...
The v0.1 package is public. Wave 2 has started from the cross-repo dogfood analysis in `docs/sprint-2026-06.md` (temporary planning artifact; sticky outcomes graduate into permanent docs at wave exit). Sprint 2.1 core is implemented and pushed. Sprint 2.2a classification work is implemented locally and verified in clean-clone smoke in commit `e8ff096`: deterministic precedence, read-time reclassification, vacuous checks, schema v2, and retired inline `human_verdict` writes. Sprint 2.2b adapter/taxonomy work is now in the working tree: provider mode is first-class, probe runs are excluded from recommendations by default, task kind is explicit or `unspecified` with a suggestion, and structured provider output parsing populates observed metadata when available. Next highest-leverage work is clean-clone verification and then Sprint 2.3 surface/friction work.
