...
The v0.1 package is public. Wave 2 has started from the cross-repo dogfood analysis in `docs/sprint-2026-06.md` (temporary planning artifact; sticky outcomes graduate into permanent docs at wave exit). Sprint 2.1 core is implemented locally: session baselines, report-to-run success linkage, and changed-file attribution cleanup. Sprint 2.2a-1 live-run status precedence is also implemented locally. Next highest-leverage non-stretch work is Sprint 2.2a-2 read-time reclassification over vendored fixtures.
