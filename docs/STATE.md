...
The v0.1 package is public. Wave 2 has started from the cross-repo dogfood analysis in `docs/sprint-2026-06.md` (temporary planning artifact; sticky outcomes graduate into permanent docs at wave exit). Sprint 2.1, Sprint 2.2a, and Sprint 2.2b are implemented and pushed. Sprint 2.3 is implemented in the working tree: TTY output is human-readable by default, `--json` preserves machine output, headless verdict capture works on `record` and `check`, provider-smoke attempts are visible, and README now leads with the five-minute loop. Next highest-leverage work is clean release verification, commit, and push.
