...
The v0.1 package is public. Wave 2 has started from the cross-repo dogfood analysis in `docs/sprint-2026-06.md` (temporary planning artifact; sticky outcomes graduate into permanent docs at wave exit). Sprint 2.1 core is implemented and pushed. Sprint 2.2a classification work is in progress locally: deterministic precedence, read-time reclassification, vacuous checks, schema v2, and retired inline `human_verdict` writes are implemented; next highest-leverage work is Sprint 2.2b adapter/taxonomy work after verification.
